...
// license that can be found in the file $GOROOT/LICENSE.

// Modifications Copyright (c) 2010 AFP Authors
// This source code is released under the terms of the
// MIT license. Please see the file LICENSE for license details.

//Modified to not export atan(), and to use float32 instead of float64
package distort
...
func xatan(arg float32) float32 {
	const (
...
func satan(arg float32) float32 {
	if arg < m.Sqrt2-1 {
		return xatan(arg)
	}
	if arg > m.Sqrt2+1 {
		return m.Pi/2 - xatan(1/arg)
	}
	return m.Pi/4 + xatan((arg-1)/(arg+1))
}
